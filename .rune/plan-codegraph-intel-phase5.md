...
## Tasks
- [x] Create `codegraph/diagram-generator.ts` — generates Mermaid diagram strings:
...
- [x] MCP tool `companion_codegraph_diagram` — agent can request architecture docs
- [ ] Web UI: Add "Architecture" tab to AI Context panel (deferred — separate UI phase)
- [ ] Telegram: `/architecture` command (deferred — separate Telegram phase)

## Acceptance Criteria
- [x] `GET /codegraph/diagram?type=architecture` returns valid Mermaid syntax
- [x] Mermaid renders correctly in standard viewers (mermaid.live)
- [ ] Web UI shows interactive architecture diagram (deferred)
- [x] MCP tool returns diagram + text description
- [x] Community labels appear as subgraph names in Mermaid
