...
/**
 * SessionView — the canonical single-session renderer.
 *
 * Renders the sub-header, context meter, message feed, composer, and
 * right-side details aside for ONE session. Used by both the fullpage route
 * (`/sessions/[id]`) and future modal overlays (Next.js intercepting route
 * over home grid). Callers own the outer container, Header chrome, and the
 * Design Preview slide panel — this component is intentionally unopinionated
 * about page-level layout so the same code can render in multiple contexts.
 *
 * Contract:
 *  - Caller supplies `sessionId`.
 *  - Caller supplies `onBack` if the back arrow should render (fullpage yes,
 *    modal usually closes instead).
 *  - Preview button lives here and triggers the global `usePreviewStore`;
 *    the actual DesignPreviewPanel is rendered by the caller so preview
 *    transitions can be page-specific.
 */
...
export interface SessionViewProps {
  sessionId: string;
  /** Shown as a back arrow in the sub-header when supplied. Omit for modal surfaces. */
  onBack?: () => void;
}
...
export function SessionView({ sessionId, onBack }: SessionViewProps) {
  ...
  const debateParticipants = useDebateStore(
  ...
  const handlePersonaSwitch = useCallback(
    async (personaId: string | null) => {
      try {
        await api.sessions.switchPersona(sessionId, personaId);
        useSessionStore.getState().setSession(sessionId, { personaId: personaId ?? undefined });
  ...
  return (
    <>
        ...
        <div className="flex min-w-0 flex-1 flex-col">
          ...
          >
            ...
            <div className="flex min-w-0 flex-1 items-center gap-2">
              ...
              <TelegramStreamBadge sessionId={sessionId} />
              {lockStatus.locked && (
                <span
                  className="inline-flex items-center gap-1 rounded-full px-2 py-0.5 text-xs"
          ...
          <MessageComposer
            ...
            onAddDebateParticipant={(model) => addDebateParticipant(sessionId, model)}
            onRemoveDebateParticipant={(modelId) => removeDebateParticipant(sessionId, modelId)}
          />
        </div>
      ...
      {shareModalOpen && (
        <ShareModal sessionId={sessionId} onClose={() => setShareModalOpen(false)} />
      )}
      {promptHistoryOpen && (
        <div
          style={{
